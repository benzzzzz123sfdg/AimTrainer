...
pygame.init()
pygame.display.set_caption('SCORE_DIFFICULT_AIMTRAINER')
...
def DESC():
    ...
    sql = "select PLAYER_NAME, ACCURACY, SCORE from leaderboard where mode='DIFFICULT' order by score desc limit 1 offset 0;"
    cur.execute(sql)
    result = cur.fetchall()
    ...
    sql = " select PLAYER_NAME, ACCURACY, SCORE from leaderboard where mode='DIFFICULT' order by score desc limit 1 offset 1;"
    cur.execute(sql)
    result = cur.fetchall()
    ...
    sql = " select PLAYER_NAME, ACCURACY, SCORE from leaderboard where mode='DIFFICULT' order by score desc limit 1 offset 2;"
    cur.execute(sql)
    result = cur.fetchall()
    ...
    while True:
        ...
        disp=font2.render("SCOREBOARD-HARD",5,black)
        screen.blit(disp, (420,100))
